...
def apply_contrast_filter(img_grayscale, show=False):
    ...
    if np.max(np.abs(laplacian_filtered)) > 1:
        print("laplacian_filtered normalized for image:", laplacian_filtered.shape, "max:", np.max(laplacian_filtered), "min:", np.min(laplacian_filtered))
        laplacian_filtered = (laplacian_filtered / np.max(np.abs(laplacian_filtered))).astype(np.float32)
...
def apply_saturation_filter(img_colored, show=False):
    ...
    if np.max(np.abs(saturation_map)) > 1:
        print("saturation_map normalized for image:", saturation_map.shape, "max:", np.max(saturation_map), "min:", np.min(saturation_map))
        saturation_map = (saturation_map / np.max(np.abs(saturation_map))).astype(np.float32)
...
def apply_well_exposedness_filter_grayscale(img_channel, sigma=0.2):
    ...
    if np.max(np.abs(we_map)) > 1:
        print("we_map normalized for image:", we_map.shape, "max:", np.max(we_map), "min:", np.min(we_map))
